    ...
    it('should be usable multiple times in streaming mode', function(done) {
      ...
      rimraf(outpath, function(err){
        ...
        var outstream = instream
          .pipe(gulp.dest(join(outpath, "test1")))
          .pipe(gulp.dest(join(outpath, "test2")))
          .pipe(gulp.dest(join(outpath, "test3")));
        ...
        outstream.on('end', function() {
          fs.readFileSync(join(outpath, "test1" ,"foo.txt"), 'utf-8')
            .should.equal('plopplopplopplop');
          fs.readFileSync(join(outpath, "test2" ,"foo.txt"), 'utf-8')
            .should.equal('plopplopplopplop');
          fs.readFileSync(join(outpath, "test3" ,"foo.txt"), 'utf-8')
            .should.equal('plopplopplopplop');
          done();
        });
        process.nextTick(function() {
